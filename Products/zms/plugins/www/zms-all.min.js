...
function zmiAutocomplete(s,o){pluginAutocomplete(s,function(){try {var prev_el=$(s).autocomplete(o).prev()[0];if($(prev_el).attr('class')!='input-group-prepend'){$(s).autocomplete(o).before('<div class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search"></i></span></div>');$(s).closest('.col-sm-10,td').wrapInner('<div class="input-group"></div>')}
} catch(error){console.error(error);$(s).autocomplete(o).before('<div class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search"></i></span></div>');$(s).closest('.col-sm-10,td').wrapInner('<div class="input-group"></div>')}
})}
...
html=v}
return html};$ZMI.registerReady(function(){$ZMI.setCursorWait("BO bootstrap.plugin.zmi");var manage_menu=typeof window.parent.frames!="undefined"&&typeof window.parent.frames.manage_menu!="undefined";$("body").each(function(){var data_root=$(this).attr('data-root');var data_path=$(this).attr('data-path');if(typeof data_root !="undefined"&&typeof data_path !="undefined"){if(manage_menu){$("#zmi-tab .breadcrumb .active").each(function(){$(this).append(' <a href="javascript:;" title="Bookmark"><i class="fas fa-bookmark-empty text-muted"></a>');var key="ZMS."+data_root+".bookmarks";var bookmarks=$ZMILocalStorageAPI.get(key,[]);$("a:last",this).click(function(){var index=bookmarks.indexOf(data_path);if(index >=0){bookmarks.splice(index,1);$('.fa-bookmark',this).removeClass("fa-bookmark text-primary").addClass("fa-bookmark-empty text-muted")}
else {bookmarks.push(data_path);$('.fa-bookmark-empty',this).removeClass("fa-bookmark-empty text-muted").addClass("fa-bookmark text-primary")}
...
});var index=bookmarks.indexOf(data_path);if(index >=0){$('.fa-bookmark-empty',this).removeClass("fa-bookmark-empty text-muted").addClass("fa-bookmark text-primary")}
else {$('.fa-bookmark',this).removeClass("fa-bookmark text-primary").addClass("fa-bookmark-empty text-muted")}
})}
...
$('a#navbar-sitemap').each(function(){var $a=$(this);if(self.window.parent.frames.length > 1&&typeof self.window.parent !="undefined"&&(self.window.parent.location+"").indexOf('dtpref_sitemap=1')> 0){$a.attr('target','_top')}
...
});$('div.single-line').each(function(){var $single_line=$(this);var $textarea=$('textarea',this);$textarea.prop({rows:1,wrap:'off'});if($single_line.hasClass("zmi-code")){$('textarea',$single_line).on('focus',function(){if(!$(this).hasClass('open-zmi-code')){$(this).addClass('open-zmi-code').removeClass('alert-warning').css('height','auto').animate({height:$(this).prop('scrollHeight')+'px'},600);if($(this).attr('data-dblclickhandler')){$(this).dblclick(function(e){$('.zmi-code-close',$single_line).remove();$('textarea',$single_line).animate({height:'30px'},0).css('height','auto').removeClass('open-zmi-code').addClass('alert-warning').blur();eval($(this).attr('data-dblclickhandler'))})}
...
})});if($(this).hasClass('form-insert')){$(".form-group label.col-lg-2 control-label.mandatory",this).each(function(){var $label=$(this);$label.prepend('<i class="fas fa-exclamation"></i>')})}
$("input,select,textarea",this).each(function(){var $this=$(this);$this.attr("data-initial-value",$this.val());$this.change(function(){if($this.val()==$this.attr("data-initial-value")){$this.removeClass("form-modified")}
else {$this.addClass("form-modified")}
})});$('input.zmi-input-icon-clazz',this).each(function(){$(this).wrap('<div class="input-group"></div>');$(this).before('<span class="input-group-text"><i class="fas fa-invisible"></i></span>');$(this).parent().children('span.input-group-text').wrap('<div class="input-group-prepend"></div>');var fn=function(){var $input=$(this);var $formGroup=$input.parents(".form-group");var $i=$("i",$formGroup);$i.attr("class",$input.val())};$(this).change(fn).keyup(fn).change()});var fn_url_input_each=function(){var $input=$(this);var fmName=$input.parents("form").attr("name");var elName=$input.attr("name");$input.wrap('<div class="input-group"></div>');var $inputgroup=$input.parent();if($input.prop("disabled")){$inputgroup.append(''+'<div class="input-group-append">'+'<span class="input-group-text"><i class="fas fa-ban-circle"></i></span>'+'</div>')}
else {$inputgroup.append(''+'<div class="input-group-append">'+'<a class="btn btn-secondary" href="javascript:;" onclick="return zmiBrowseObjs(\''+fmName+'\',\''+elName+'\',getZMILang())">'+'<i class="fas fa-link"></i>'+'</a>'+'</div>')}
...
$("input.url-input",$inputContainer).each(fn_url_input_each);$(".input-group-append",$inputContainer).replaceWith('<a class="btn btn-secondary" href="javascript:;"><i class="fas fa-times"></i></a>');$("input.url-input",$inputContainer).next().click(function(){$(this).parents(".input-group").next(".breadcrumb").remove();$(this).parents(".input-group").remove();var l=[];$("input",$inputContainer).each(function(){var v=$(this).val();if(v.length > 0){l.push(v)}
...
$("input.datepicker,input.datetimepicker",this).each(function(){$(this).closest("div").addClass("input-group").removeClass("col-sm-9").removeClass("col-sm-10").removeClass("col-md-10").wrap('<div class="col-sm-4 col-md-3 col-lg-3"></div>');$(this).before('<div class="input-group-prepend"><span class="input-group-text"><i class="far fa-calendar"></i></span></div>')})});if(typeof this.afterInitInputFieldsHandler !="undefined"){for(var i in this.afterInitInputFieldsHandler){this.afterInitInputFieldsHandler[i]()}}
...
return false}
ZMIObjectTree=function(){};$ZMI.objectTree=new ZMIObjectTree();ZMIObjectTree.prototype.init=function(s,href,p){var that=this;that.p=p;that.active=[];href=href+"/"+(typeof p['init.href'] !="undefined"?p['init.href']:"ajaxGetParentNodes");$(s).html('<i class="fas fa-spinner fa-spin"></i>&nbsp;'+getZMILangStr('MSG_LOADING'));var params={lang:getZMILang(),preview:'preview'};if(typeof that.p["params"]=="object"){for(var i in that.p["params"]){params[i]=that.p["params"][i]}}
...
};html+='<ul data-id="'+page_id+'" data-home-id="'+page_home_id+'" class="zmi-page '+page_meta_type+'">';html+='<li class="'+css.join(' ')+'">';html+=$ZMI.icon("fas fa-caret-right toggle",'title="+" onclick="$ZMI.objectTree.toggleClick(this'+(typeof callback=="undefined"?'':','+callback)+')"')+' ';if(page_is_pageelement){html+='<span style="cursor:help" onclick="$ZMI.objectTree.previewClick(this)" title="'+$ZMI.getLangStr('TAB_PREVIEW')+'"><i class="'+page_icon+'"></i></span> '}
html+='<a href="'+page_absolute_url+'"'+' data-link-url="'+link_url+'"'+' data-uid="'+page_uid+'"'+' data-page-physical-path="'+page_physical_path+'"'+' data-anchor="'+anchor+'"'+' data-page-is-page="'+page_is_page+'"'+' data-page-titlealt="'+page_titlealt.replace(/"/g,'&quot;').replace(/'/g,'&apos;')+'"'+' onclick="return zmiSelectObject(this)">';if(!page_is_pageelement){html+='<i class="'+page_icon+'"></i> '}
html+=page_titlealt;html+='</a>';html+='</li>';html+='</ul><!--.zmi-page-->'}
return html}
ZMIObjectTree.prototype.toggleClick=function(toggle,callback){var that=this;var $container=$(toggle).parents("li:first");$container.children("ul").remove();if($(toggle).hasClass("fa-caret-right")){$(toggle).removeClass("fa-caret-right").addClass("fa-caret-down").attr({title:'-'});var href='';var homeId=null;$(toggle).parents(".zmi-page").each(function(){var dataId=$(this).attr("data-id");var dataHomeId=$(this).attr("data-home-id");if(homeId==null){homeId=dataHomeId}
...
var base=$ZMI.getPhysicalPath();base=base.substr(0,base.indexOf('/'+homeId));$container.append('<div id="loading" class="zmi-page"><i class="fas fa-spinner fa-spin"></i>&nbsp;&nbsp;'+getZMILangStr('MSG_LOADING')+'</div>');var params={lang:getZMILang(),preview:'preview',physical_path:$('meta[name=physical_path]').attr('content')}
if(typeof that.p["params"]=="object"){for(var i in that.p["params"]){params[i]=that.p["params"][i]}}
$.get(base+href+'/manage_ajaxGetChildNodes',params,function(result){$("#loading").remove();var pages=$('page',result);if(pages.length==0){$(toggle).removeClass("fa-caret-down").attr({title:''})}
...
return false});var action=zmiParams['base_url'];action=action.substr(0,action.lastIndexOf("/"));action+="/manage_ajaxZMIActions";var params={};params['lang']=lang;params['context_id']=context_id;$.get(action,params,function(data){var value=eval('('+data+')');var id=value['id'].replace(/\./,"_");var actions=value['actions'];$(el).append('<div class="dropdown-menu"></div>');$ul=$(".dropdown-menu",el);var startsWithSubmenu=actions.length > 1&&actions[1][0].indexOf("-----")==0&&actions[1][0].lastIndexOf("-----")> 0;console.log("[ZMIActionList.over]:startsWithSubmenu="+startsWithSubmenu);var o=0;if(startsWithSubmenu){o=2;var html='';var action=actions[1];var optlabel=action[0];var opticon='<i class="fas fa-plus-sign"></i>';optlabel=optlabel.substr("-----".length);optlabel=optlabel.substr(0,optlabel.lastIndexOf("-----"));optlabel=optlabel.basicTrim();$("button.split-left",el).html(opticon+' '+optlabel)}
...
var action=actions[i];var optlabel=action[0];var optvalue=action[1];var opticon=action.length>2?action[2]:'';var opttitle=action.length>3?action[3]:'';if(optlabel.indexOf("-----")==0&&optlabel.lastIndexOf("-----")> 0){opticon='<i class="fas fa-caret-down"></i>';optlabel=optlabel.substr("-----".length);optlabel=optlabel.substr(0,optlabel.lastIndexOf("-----"));optlabel=optlabel.basicTrim();$ul.append('<div class="dropdown-header '+optvalue+'">'+opticon+' '+optlabel+'</div>')}
...
$('.scroller-right').click(function(){$('.scroller-left').fadeIn('slow');$('.scroller-right').fadeOut('slow');$('.wrapper .nav.nav-tabs').animate({left:"+="+widthOfHidden()+"px"},'slow',function(){})});$('.scroller-left').click(function(){$('.scroller-right').fadeIn('slow');$('.scroller-left').fadeOut('slow');$('.wrapper .nav.nav-tabs').animate({left:"-="+getLeftPosi()+"px"},'slow',function(){})});reAdjust();$(window).on('resize',function(e){reAdjust()})});$ZMI.registerReady(function(){const scrollToTopButton=document.getElementById('js-top');if(scrollToTopButton){const scrollFunc=()=> {let y=window.scrollY;if(y > 0){scrollToTopButton.className="back-to-top show"} else {scrollToTopButton.className="back-to-top hide"}
};window.addEventListener("scroll",scrollFunc);const scrollToTop=()=> {const c=document.documentElement.scrollTop||document.body.scrollTop;if(c > 0){window.requestAnimationFrame(scrollToTop);window.scrollTo(0,c-c / 10)}}
